...
class AppConfig:
    ...
    dbus_name: str = "com.openclaw.LinuxAgentShell"
    dbus_path: str = "/com/openclaw/LinuxAgentShell"

    @classmethod
    def default(cls, root: Path | None = None) -> "AppConfig":
        project_root = root or Path(__file__).resolve().parents[1]
        runtime_dir = Path.home() / ".local" / "state" / "linux-agent-shell"
        config_dir = Path.home() / ".config" / "linux-agent-shell"
        return cls(
            root=project_root,
